...
def page_credit_request():
    ...
    with tab2:
        ...
        column_config = {
            ...
            "GENDER": st.column_config.SelectColumn("Género", options=['Masculino', 'Femenino'], required=True),
            "CNT_CHILDREN": st.column_config.SelectColumn("Hijos", options=['0','1', '2', '3', '4 o más'], required=True),
            "EDUCATION": st.column_config.SelectColumn("Educación", options=['Lower secondary', 'Secondary / secondary special', 'Incomplete higher', 'Higher education', 'Academic degree'], required=True),
            "FAMILY_STATUS": st.column_config.SelectColumn("Estado Civil", options=['Married', 'Single / not married', 'Civil marriage', 'Separated', 'Widow'], required=True),
            "HOUSING": st.column_config.SelectColumn("Vivienda", options=['With parents', 'Rented apartment', 'House / apartment', 'Municipal apartment', 'Office apartment', 'Co-op apartment'], required=True),
            "INCOME_TYPE": st.column_config.SelectColumn("Fuente Ingresos", options=['Working', 'State servant', 'Commercial associate', 'Businessman', 'Maternity leave', 'Student', 'Unemployed', 'Pensioner'], required=True),
